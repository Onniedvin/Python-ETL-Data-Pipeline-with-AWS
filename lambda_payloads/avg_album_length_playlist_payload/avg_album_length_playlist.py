...
PLAYLIST = 'struttin' # My personal playlist
...
def gather_data_local():
    ...
    with open("albums.csv", 'w') as file:
        ...
        for artist in list(artists.keys()):
            print(artist)
            artists_albums = spotipy_object.artist_albums(artist, album_type='album', limit=10)
            # For all of their albums
            for album in artists_albums['items']:
                if 'FI' in album['available_markets']: # Filtering only Finland
...
def gather_data():
    # For every artist we're looking for
    with open("albums.csv", 'w') as file:
        header = ['Year Released', 'Album Length (ms)', 'Album Name', 'Artist']
        writer = csv.DictWriter(file, fieldnames=header)
        writer.writeheader()
        artists = get_artists_from_playlist(spotify_playlists()[PLAYLIST])
        for artist in artists.keys():
            ...
            # For all of their albums
            for album in artists_albums['items']:
                if 'FI' in artists_albums['items'][0]['available_markets']:
                    album_data = spotipy_object.album(album['uri'])
                    # For every song in the album
                    album_length_ms = 0
                    for song in album_data['tracks']['items']:
    ...
    response = s3_resource.Object('spotify-analysis-data', filename).upload_file("/tmp/albums.csv")
